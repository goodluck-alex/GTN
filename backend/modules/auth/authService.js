...
function shouldSkipGeoIpLookup(reqIp) {
  ...
  // Typical private / local ranges — no reliable public geo; default East Africa dial prefix.
...
async function detectCountryPrefixFromIp(reqIp) {
  if (shouldSkipGeoIpLookup(reqIp)) return "+256";
  const ip = String(reqIp || "").trim();
...
export async function createUserAccount(
  ...
  if (!finalPhone) {
    ...
    const countryPrefix = await detectCountryPrefixFromIp(ip);
